...
function findIntermediateStations(source, destination) {
  ...
  if (lowercaseSource === lowercaseDestination) {
    return {
      error:
        "Source and destination are the same. Please choose different locations.",
    };
  }

  const sourceIndex = stations.findIndex((station) =>
    station.value ? station.value.toUpperCase() === lowercaseSource : false
  );
